...
/**
     ...
     */
export const useCharacters = (pageNum = 1, characterName = "") => {
  const [characters, setUrl] = useFetch(mainUrls.characters + pageNum + `&name=${characterName}`);
  useEffect(() => {
    setUrl(mainUrls.characters + pageNum + `&name=${characterName}`);
  }, [pageNum, characterName]);
